...
async function fetchPlayerStats(name, platform) {
  ...
  const url = `${FN_API}?name=${encodeURIComponent(name)}&accountType=${encodeURIComponent(platform)}&image=all`;
  const res = await fetch(url, { headers: { Authorization: key } });
  if (!res.ok) {
    if (res.status === 403) throw new Error('That API key was rejected. Double-check it in the API Key tab.');
